...
torch.nn 은 클래스이고, torch.nn.funtional은 함수이다.

torch.nn으로 구현한 클래스의 경우에는 attribute를 활용해 state를 저장하고 활용할 수 있고 torch.nn.functional로 구현한 함수의 경우에는 인스턴스화 시킬 필요 없이 사용이 가능하다.

**"????"**

무슨말인지 모르겠다면 파이썬을 조금더 공부할 필요가 있다. 필자가 그랬다 ^^

<p align="center"><img src="/MYPICS/lec04/python-1.jpg" width = "600" ></p>

클래스는 빵틀이라고 보면 편하다. 오브젝트(인스턴스,instance)는 클래스 즉, 빵틀로 찍어낸 빵이라고 생각하면 이해하기 쉬울 것이다.

인터넷에 Class 와 Instance 의 좋은예가 있어서 소개해드리려합니다.
...
출처: https://blockdmask.tistory.com/443 [개발자 지망생:티스토리]
```

결과값

<p align="center"><img src="/MYPICS/lec04/class.jpeg" width = "600" ></p>



## 파이썬 클래스로 파이토치 모델 구현하기
