...
**💻 해결 방법 2** : Windows 시스템 로캘 변경
개발용 PC라면, Windows 시스템 전체의 언어 지원을 UTF-8로 변경하는 것이 장기적으로 좋습니다.
...
해서, 다시 부트로더부터 구워버릴까 싶어 nRF Connect for Desktop을 열어서 HW에 접근하려하면 아래와 같이 에러가 발생합니다.
(이제와서 생각해보니, recover 명령어도 작동하지 않는 상태인데 될리가 없는데..😂)
...
이런 현상이 있을 때는 정말 아무것도 할 수 있는게 없습니다.
